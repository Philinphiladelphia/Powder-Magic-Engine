...
void DebugLines::Draw()
{
	...
	if (view->GetDrawingLine())
	{
		...
		PTString info;
		info = PTString::Build(drawPoint2.X, " x ", drawPoint2.Y);
		...
		info = PTString::Build(drawPoint1.X, " x ", drawPoint1.Y);
		...
		info = PTString::Build(std::abs(drawPoint2.X-drawPoint1.X));
		...
		info = PTString::Build(std::abs(drawPoint2.Y-drawPoint1.Y));
