...
void SearchModel::BeginSearchSaves(int start, int count, PTString query, http::Period period, http::Sort sort, http::Category category)
{
	lastError = "";
	resultCount = 0;
...
void SearchModel::BeginGetTags(int start, int count, PTString query)
{
	lastError = "";
...
bool SearchModel::UpdateSaveList(int pageNumber, PTString query)
{
	//Threading
	if (!searchSaves)
	{
