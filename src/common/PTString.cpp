...
PTString ByteString::FromUtf8(bool ignoreError) const
{
	std::vector<PTString::value_type> destination;
	...
	return PTString(destination.data(), destination.size());
}

std::vector<PTString> PTString::PartitionBy(value_type ch, bool includeEmpty) const
{
	std::vector<PTString> result;
	size_t at = 0;
	while(true)
	{
		Split split = SplitBy(ch, at);
		PTString part = split.Before();
		if(includeEmpty || part.size())
			result.push_back(part);
		at = split.PositionAfter();
		if(!split)
...
std::vector<PTString> PTString::PartitionBy(PTString const &str, bool includeEmpty) const
{
	std::vector<PTString> result;
	size_t at = 0;
	while(true)
...
std::vector<PTString> PTString::PartitionByAny(PTString const &str, bool includeEmpty) const
{
	std::vector<PTString> result;
	size_t at = 0;
	while(true)
...
PTString &PTString::Substitute(PTString const &needle, PTString const &replacement)
{
	size_t needleSize = needle.size();
	size_t replacementSize = replacement.size();
	size_t at = super::find(needle);
...
ByteString PTString::ToUtf8() const
{
...
inline PTString::value_type widen_wchar(wchar_t ch)
{
	return std::make_unsigned<wchar_t>::type(ch);
}

inline bool representable_wchar(PTString::value_type ch)
{
	return ch < PTString::value_type(std::make_unsigned<wchar_t>::type(std::numeric_limits<wchar_t>::max()));
}

inline wchar_t narrow_wchar(PTString::value_type ch)
{
	return wchar_t(ch);
}
...
PTString numberString(numberChars);
...
struct LocaleImpl
{
	...
	inline void PrepareWStream(PTString const &str, size_t pos, std::ios_base::fmtflags set, std::ios_base::fmtflags reset)
	{
	...
	inline void FlushWStream(StringBuilder &b)
	{
		...
		std::vector<PTString::value_type> chars; // operator new?
...
PTString StringBuilder::Build() const
{
	return PTString(buffer.begin(), buffer.end());
}

void StringBuilder::AddChars(PTString::value_type const *data, size_t count)
{
	buffer.reserve(buffer.size() + count);
	buffer.insert(buffer.end(), data, data + count);
}
...
{
	PTString::value_type ch = data;
	b.AddChars(&ch, 1);
	return b;
}

StringBuilder &operator<<(StringBuilder &b, PTString::value_type data)
{
	b.AddChars(&data, 1);
	return b;
}

StringBuilder &operator<<(StringBuilder &b, PTString::value_type const *data)
{
	return b << PTString(data);
}

StringBuilder &operator<<(StringBuilder &b, PTString const &data)
{
	b.AddChars(data.data(), data.size());
	return b;
}
...
PTString::Split PTString::SplitSigned(long long int &value, size_t pos, std::ios_base::fmtflags set, std::ios_base::fmtflags reset) const
{
	LocaleImpl *impl = getLocaleImpl();
...
PTString::Split PTString::SplitUnsigned(unsigned long long int &value, size_t pos, std::ios_base::fmtflags set, std::ios_base::fmtflags reset) const
{
	LocaleImpl *impl = getLocaleImpl();
	impl->PrepareWStream(*this, pos, set, reset);
	impl->wstream >> value;
...
PTString::Split PTString::SplitFloat(double &value, size_t pos, std::ios_base::fmtflags set, std::ios_base::fmtflags reset) const
{
	LocaleImpl *impl = getLocaleImpl();
	impl->PrepareWStream(*this, pos, set, reset);
	impl->wstream >> value;
